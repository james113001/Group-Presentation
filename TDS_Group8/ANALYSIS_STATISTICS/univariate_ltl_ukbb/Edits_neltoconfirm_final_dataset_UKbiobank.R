...
## NEL: I added more variables to delete to avoid repetition of same info


df<- data %>% filter(is.na(Z_adj_TS)==FALSE) %>% 
  select(-c("Age_recruited","smoking_status","Unadj_TS","Adj_TS",
            "TS_reg_dil", "Age_diabetes_bin", "MenopauseAge_bin", "source_dementia", "Syst_BP", "Dia_BP")) %>% 
  mutate(
...
Ethnicity=fct_recode(Ethnicity, NULL = "DN/PntA")
                ...
                "IMD_town","Dia_BP_auto","Syt_BP_auto","lymp_perc")) %>%
  mutate(
...
## Izzy: I added this to change factors with 0 / 1 to yes / no
...
#Izzy added this: relevel so reference is the "norm"/ without condition 
...
#Izzy added this: Variable groups:
