...
dff4<-dff3 %>% select(-Leukocyte_telomere_length) %>% select(-eid)


#for when submitting back on:
...
Xvar_cts<-data.frame(pvalues[,!sapply(pvalues,is.factor)] %>% select(-Leukocyte_telomere_length))
...
Betas_pvals<-function(X){ 
  model1 = summary(lm(Leukocyte_telomere_length ~ X, data=pvalues))$coefficients
...
lrt__a<- function(Xvar) {
  mod<-summary(lm(Leukocyte_telomere_length ~ Xvar, data=pvalues))
...
ltl_results<- ltl_results %>% mutate(Variable= rownames(ltl_results))

#Keep factors of ones without multi levels

separated_ltl<-separate(ltl_results,Variable,into =c("Variable","factor"), sep=".X")
separated_ltl$Variables <- gsub("^.", "", separated_ltl$Variables)
separated_ltl = subset(separated_ltl, select = -c(factor) )                            
separated_ltl$factor = separated_ltl$Variables
separated_ltl = subset(separated_ltl, select = -c(Variables) )                            
...
saveRDS(data.frame(separated_ltl), paste0("../ANALYSIS_STATISTICS/univariate_ltl_ukbb/Results_univ_reg/univariate_ltl.rds"))
