...
dff4<-dff3 %>% select(-Z_adj_TS) %>% select(-eid) %>% select(-Sex) %>% select(-Age)
ids=as.character(cut(1:length(dff4)-3, breaks = nchunks, labels = 1:nchunks))

#dff4[,ids == ichunk] <- sub this back in the dataframe below when submiting is back on
pvalues=data.frame(dff4 , "Z_adj_TS" = dff3 %>% 
                     select(Z_adj_TS) %>% 
...
Xvar_cts<-data.frame(pvalues[,!sapply(pvalues,is.factor)] %>% select(-Z_adj_TS, -Age))

Xvar_factors<- data.frame(pvalues[,sapply(pvalues,is.factor)] %>% select(-Sex))


#Check each variable one by one to see which significant:

Betas_pvals<-function(X){
  model1 = summary(lm(Z_adj_TS ~ X + Age + Sex, data=pvalues))$coefficients
...
separated_ltl<-left_join(separated_ltl,subgroup, by = "Variable") %>% mutate(factor=ifelse(is.na(factor),Variable,factor)) %>% 
  ...
  filter(!grepl('.SexMale', Variable))



saveRDS(data.frame(separated_ltl), paste0("../ANALYSIS_STATISTICS/univariate_ltl_ukbb/Results_univ_reg/univariate_ltl_adjusted.rds"))
