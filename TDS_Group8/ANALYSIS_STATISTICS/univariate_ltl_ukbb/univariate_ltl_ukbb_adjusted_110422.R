...
dff4<-dff3 %>% select(-Leukocyte_telomere_length) %>% select(-eid) %>% select(-Sex) %>% select(-Age)
...
Betas_pvals<-function(X){
  model1 = summary(lm(Leukocyte_telomere_length ~ X + Age + Sex, data=pvalues))$coefficients
...
saveRDS(data.frame(separated_ltl), paste0("../ANALYSIS_STATISTICS/univariate_ltl_ukbb/Results_univ_reg/univariate_ltl_adjusted_update.rds"))
